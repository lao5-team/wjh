...
import com.test.art.adapter.ActivityMemberSelectAdapter;
import com.test.art.adapter.ActivityMemberStateAdapter;
import com.test.art.data.ActivityData;
import com.test.art.data.MyUser;
import com.test.art.domain.activity.ActivityManager;
import com.test.art.widget.Sidebar;
...
public class ActivityMembersActivity extends FragmentActivity {
	...
	private List<com.test.art.domain.User> mContactList = new ArrayList<com.test.art.domain.User>();;
...
//	public void addFriend(com.test.art.domain.User user)
//	{
...
//	}
//	
//	public void removeFriend(com.test.art.domain.User user)
//	{
//		for(int i=0; i<mFriends.size(); i++)
//		{
//			if(mFriends.get(i).mName.equals(user.getNick()))
...
//	}	
	private void getContactList() {
		mContactList.clear();
		Map<String, com.test.art.domain.User> users = DemoApplication.getInstance().getContactList();
		Iterator<Entry<String, com.test.art.domain.User>> iterator = users.entrySet().iterator();
		while (iterator.hasNext()) {
			Entry<String, com.test.art.domain.User> entry = iterator.next();
			if (!entry.getKey().equals(Constant.NEW_FRIENDS_USERNAME) && !entry.getKey().equals(Constant.GROUP_USERNAME))
				mContactList.add(entry.getValue());
		}
		// 排序
		Collections.sort(mContactList, new Comparator<com.test.art.domain.User>() {

			@Override
			public int compare(com.test.art.domain.User lhs, com.test.art.domain.User rhs) {
